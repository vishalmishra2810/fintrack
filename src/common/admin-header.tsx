    ...
    return (
        <header className="bg-blue-500 py-4">
            <div className="container mx-auto flex justify-between items-center">
                <a href="/">
                <h1 className="text-white text-2xl font-bold">Your Logo</h1></a>
                <nav className="flex space-x-4">
                    ...
                    {categories.map(category => (
                        <div key={category.id} className="relative group">
                            <span className="text-white cursor-pointer group-hover:text-gray-200">{category.name}</span>
                            <ul className="absolute hidden bg-white border border-gray-300 rounded-md shadow-md p-2 mt-1 group-hover:block">
                                {category.subcategories.map(subcategory => (
                                    <li key={subcategory} className="cursor-pointer hover:text-blue-500">{subcategory}</li>
                                ))}
                            </ul>
                        </div>
                    ))}
